...
class Mailjet
{
    ...
    public function __call($method, $args) {
        ...
        # params
        $params = (count($args) > 0) ? current($args) : array();

        # request method
        $request = isset($params["method"]) ? \Thin\Inflector::upper($params["method"]) : 'GET';
    ...
    public function requestUrlBuilder($method, $params = array(), $request)
    {
        ...
        foreach($params as $key => $value) {
            if($request == "GET" || \Thin\Arrays::inArray($key, array('apikey', 'output'))) $query_string[$key] = $key.'=' . urlencode($value);
    ...
    public function debug() {
        ...
        if(isset($this->_response_code)) :
            ...
            if($this->_response_code == 200) :
                ...
                if(isset($this->_response)) :
                    echo '<tr><th>Response</th><td><pre>' . utf8_decode(print_r($this->_response, true)) . '</pre></td></tr>';
                endif;
        ...
        foreach($args as $arg) {
            $arg = explode("=", $arg);
            echo current($arg) . ' = <span style="color:#ff6e56;">' . end($arg) . '</span><br/>';
        }
